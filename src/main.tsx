...
import { publicRoutes } from "./seo/site.ts";
import { ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';

function Protected({ children }: { children: ReactNode }) {
  const { loading, profileLoading, isAuthenticated, onboardingState } =
    useAuth();
  if (loading || profileLoading || onboardingState === "loading")
    return <div className="state-panel">Loading your account...</div>;
  if (!isAuthenticated) return <Navigate to="/auth" replace />;
  if (onboardingState !== "complete") return <Navigate to="/" replace />;
  return children;
}
function OnboardingRoute() {
  const { isAuthenticated } = useAuth();
  return isAuthenticated ? <Navigate to="/" replace /> : <OnboardingPage />;
}
      ...
      <BrowserRouter>
        <Routes>
          <Route element={<AppShell />}>
            ...
            <Route path={publicRoutes.trailers} element={<TrailersPage />} />
            <Route
