...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setStatus("loading");
      try {
        const items = await searchAnime(term);
  ...
  return (
    <section className="search-page">
      ...
      <div className="search-form">
        <label htmlFor="anime-search">Search titles</label>
        <div>
          <input
            id="anime-search"
            type="search"
            ...
            placeholder="Try English, Romaji, or a native title"
            autoComplete="off"
          />
          {query && (
            <button
      ...
      {status === "error" && (
        <div className="state-panel">
          <h2>We couldn't complete that search right now.</h2>
      ...
      {status === "success" && results.length === 0 && (
        <div className="state-panel">
          ...
          <p>Try another title, spelling, or native title.</p>
        </div>
      )}
      {status === "success" && results.length > 0 && (
            ...
            <Link className="anime-card" to={`/anime/${item.id}`} key={item.id}>
              ...
              <div className="anime-card-body">
                ...
                {item.nativeTitle && item.nativeTitle !== item.title && (
                  <p className="anime-alt-title">{item.nativeTitle}</p>
                )}
