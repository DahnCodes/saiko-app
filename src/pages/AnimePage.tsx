  ...
  return (
    <section className="anime-page">
      <div className="section-heading">
        <div>
          <p className="eyebrow">The watchlist starts here</p>
          <h1>Top anime</h1>
        </div>
        <p className="section-intro">
          A living shelf of the series and films shaping the conversation.
        </p>
      </div>
      {status === "loading" && (
        <div className="anime-grid">
          {Array.from({ length: 6 }, (_, index) => (
            <div className="anime-card skeleton" key={index} />
          ))}
        </div>
      )}
      {status === "error" && (
        <div className="state-panel">
          ...
          <p>We could not load anime right now. Please try again shortly.</p>
        </div>
      )}
