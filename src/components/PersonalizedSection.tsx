...
export default function PersonalizedSection() {
  ...
  if (!user || !profile?.onboardingCompleted || !items.length) return null;
  return (
    <section className="home-section">
      <div className="section-heading">
        <div>
          <p className="eyebrow">Welcome back, @{profile.username}</p>
          <h2>For you</h2>
        </div>
        <Link className="section-link" to="/recommendations">
          View all
        </Link>
      </div>
