...
export async function searchAnime(query: string, limit = 12): Promise<Anime[]> { const normalized = query.normalize('NFKC').replace(/[^\p{L}\p{N}]+/gu, ' ').replace(/\s+/g, ' ').trim(); if (!normalized) return []; const { data, error } = await ensureClient().functions.invoke('search-anime', { body: { query: normalized, limit } }); if (error) throw error; if (data?.error) throw new Error(data.error); return ((data?.results ?? []) as AnimeRow[]).map(mapAnime) }
...
const starterQueries = ['Naruto', 'Bleach', 'One Piece', 'Demon Slayer: Kimetsu no Yaiba', 'Attack on Titan', 'My Hero Academia'] as const
function titleValues(anime: Anime) { return [anime.title, anime.englishTitle, anime.romajiTitle, anime.nativeTitle, ...anime.synonyms].filter((value): value is string => Boolean(value)).map((value) => value.toLowerCase()) }
export async function getStarterAnime(): Promise<Anime[]> { const groups = await Promise.all(starterQueries.map((query) => searchAnime(query, 12))); return groups.map((items, index) => { const query = starterQueries[index].toLowerCase(); const aliases = query.startsWith('demon slayer') ? ['demon slayer', 'kimetsu no yaiba'] : [query]; return items.find((anime) => titleValues(anime).some((value) => aliases.some((alias) => value === alias || value.startsWith(`${alias}:`)))) ?? items.find((anime) => titleValues(anime).some((value) => aliases.some((alias) => value.includes(alias)))) }).filter((anime): anime is Anime => Boolean(anime)) }
